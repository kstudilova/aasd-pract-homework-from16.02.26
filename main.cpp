...
1) Может быть выход за границы выходного массива,
2) Нет гарантии, что входные массивы отсортированы
*/
...
T* mergeN(const T* const* a, size_t sa, const size_t* sai, T* c)
{
  ...
  for (size_t i = 1; i < sa; ++i)
  {
    T* tmp = nullptr;
    try {
      tmp = new T[totalSize + sai[i]]; //T::T()
      size_t k = 0;
      size_t indexC = 0;
      size_t indexA = 0;
      while (indexC < totalSize && indexA < sai[i])
      {
        if (c[indexC] < a[i][indexA]) //operator<
        {
          tmp[k++] = c[indexC++]; //operator=
        } else {
          tmp[k++] = a[i][indexA++]; //operator=
        }
      }
